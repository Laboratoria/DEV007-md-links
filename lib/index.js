...
const mdLinks = (path, options) => new Promise((resolve, reject) => {
  if (checkPathExists(path)) {
    console.log(chalk.bgMagenta.bold('La ruta existe'));
    ...
    if (IsDirectory(path)) {
      ...
      console.log(chalk.bgMagenta.bold('es un directorio'), arrayMd);
    } else {
      arrayMd = isMd(path);
    ...
    console.log(chalk.bgMagenta.bold('la ruta no existe'));
    resolve([]);
  }
});
